...
function seleccionarProducto(productoId) {
    ...
    $("#perfAroma").val(productoId).change();
}
  
function cargarSelect(producto) {
    var option = `<option value="${producto.id}">${producto.nombre}</option>`;
    $("#perfAroma").append(option);
}
  
function agregarPrecio() {
    $("#error").html("");
    var valor = $("#perfAroma option:selected").val();
...
function agregarProducto() {
    ...
    if (cantidad > 0) {
      $("#error").html("");
      var itemId = parseInt($("#perfAroma").val());
...
$("#pedidoPrincipal").on("click", function () {
  let posicion = $("#customer").offset().top;
